...
/*   Updated: 2025/04/27 13:39:03 by zsonie           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_path_and_size(t_gameenv *env)
{
	...
	env->img.player_exit_path = "./textures/player_exit32.xpm";
	env->img.exit_path = "./textures/exit32.xpm";
}

static int	texture_check(t_gameenv *env, void **img, char *path)
{
	int check;

	check = open(path, STDOUT_FILENO);
	if (check < 0)
		return (1);
...
int	init_img_from_xpm(t_gameenv *env)
{
	...
	if (texture_check(env, &env->img.img_wall, env->img.wall_path))
		return (1);
	if (texture_check(env, &env->img.img_ground, env->img.ground_path))
		return (1);
	if (texture_check(env, &env->img.img_exit, env->img.exit_path))
		return (1);
	if (texture_check(env, &env->img.img_collect, env->img.coin_path))
		return (1);
	if (texture_check(env, &env->img.img_wall, env->img.wall_path))
		return (1);
	if (texture_check(env, &env->img.img_player_exit, 
		env->img.player_exit_path))
		return (1);
	if (texture_check(env, &env->img.img_player, env->img.player_path))
		return (1);
	return (0);
}
